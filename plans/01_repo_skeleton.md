...
2. `src/stick_idr/` tree of stub modules per §3 of `IMPLEMENTATION_PLAN.md`.
3. `src/stick_idr/__init__.py` exporting the public surface (stubs).
4. `src/stick_idr/doctor.py` — JAX backend probe + minimal sanity check.
5. `src/stick_idr/cli.py` — Typer CLI with `fit`, `eval`, `sim`, `figure`, `table`, `doctor`.
...
- `python -c "import stick_idr; stick_idr.doctor()"` exits 0.
