...
fig.suptitle(
    "(Illustrative synthetic placeholder.) idr-vs-realized-FDR calibration across simulation regimes (K=5, n=10,000). "
    "Solid diagonal = perfect calibration. Real numerical values to be reported in the companion experimental paper.",
    fontsize=8.5, y=1.06
)
plt.savefig(OUT_DIR / "fig_idr_calibration.pdf")
plt.close()
...
fig.suptitle(
    "(Illustrative synthetic placeholder.) Discriminative performance under heavy-tailed simulation regime S5. "
    "Curves are synthetic and indicate qualitatively expected ordering; real curves to follow.",
    fontsize=8.5, y=1.04
)
plt.savefig(OUT_DIR / "fig_roc_pr.pdf")
plt.close()
...
ax.set_title(r"(a) Posterior contraction, $\beta=1.5$, $K=5$")
...
fig.suptitle(
    "(Illustrative synthetic placeholder.) Empirical posterior contraction matching the theoretical rate. "
    "Numerical values are synthetic and follow the predicted scaling law.",
    fontsize=8.5, y=1.04
)
plt.savefig(OUT_DIR / "fig_contraction.pdf")
plt.close()
...
fig.suptitle(
    "(Illustrative synthetic placeholder.) PY versus DP cluster behaviour. PY induces a heavier tail of small clusters that captures rare dependence regimes.",
    fontsize=8.5, y=1.02
)
plt.savefig(OUT_DIR / "fig_py_vs_dp.pdf")
plt.close()
...
fig.suptitle(
    "(Illustrative synthetic placeholder.) Real-data evaluation. Counts and posterior summaries are placeholder values; actual numbers will be reported in the companion paper.",
    fontsize=8.5, y=1.02
)
plt.savefig(OUT_DIR / "fig_realdata.pdf")
plt.close()
...
fig.suptitle(
    "(Illustrative synthetic placeholder.) Runtime and mixing scaling. Numbers are synthetic and reflect the analytic complexity bound.",
    fontsize=8.5, y=1.02
)
