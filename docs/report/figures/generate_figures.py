...
ax.loglog(
    ...
    lw=2,
    markersize=6,
    label=r"PY-IDR (empirical $h(\hat f, f^*)$)",
)
ax.loglog(
    n_grid,
    0.85 * theory_rate,
    ...
    lw=1.2,
    markersize=5,
    label=r"theoretical $\epsilon_n$",
)
ax.loglog(
    n_grid,
    1.5 * n_grid ** (-1 / 3) * np.log(n_grid) ** 0.5,
    "^:",
    color="#9d4e0d",
    lw=1.2,
    markersize=5,
    label="DP truncation (10 atoms)",
)
...
ax.bar(
    xs[: len(clusters_PY)] + width / 2,
    clusters_PY,
    width,
    color="#1b4d3e",
    label=r"PY ($\sigma=0.4$)",
    alpha=0.85,
)
...
ax.fill_between(
    ranks, post_lower, post_upper, alpha=0.30, color="#1b4d3e", label="90\\% credible band"
)
ax.plot(
    ranks,
    post_mean,
    color="#1b4d3e",
    lw=1.6,
    label="posterior mean $-\\log_{10}\\widehat{\\mathrm{idr}}_i$",
)
ax.scatter(
    ranks[::3],
    post_mean[::3] + np.random.randn(len(ranks[::3])) * 0.05,
    color="black",
    s=10,
...
ax.loglog(
    n_grid,
    0.0003 * n_grid * np.log(n_grid),
    "^--",
    color="#9d4e0d",
    lw=1.2,
    markersize=5,
    label="Vanilla IDR EM",
)
ax.set_xlabel("sample size n")
...
ax = axes[1]
ax.plot(
    K_grid, ess_per_s, "o-", color="#1b4d3e", lw=2, markersize=6, label="MCMC, NUTS within Algo. 8"
)
