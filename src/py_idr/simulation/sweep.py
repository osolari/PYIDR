...
def run_sweep_ecv(
    ...
    *,
    ...
    """ECV sweep — comparator companion to :func:`run_sweep`.

    Same ``(regime, K, n, num_replicates, base_seed)`` shape as
    :func:`run_sweep`; rows carry ``method = "eCV"`` so they drop
    into the same CSV alongside PY-IDR / Vanilla IDR / MaxRank rows.

    Like MaxRank, the eCV statistic is deterministic given the data, so
    all replicates sharing a seed produce identical idr arrays — the
    per-alpha rows differ only in the Sun-Cai cutoff and the resulting
    decision metadata.
    """
    import time as _time
...
def run_sweep_chipr(
    ...
    *,
    ...
    """ChIP-R sweep — comparator companion to :func:`run_sweep`.

    Same ``(regime, K, n, num_replicates, base_seed)`` shape as
    :func:`run_sweep`; rows carry ``method = "ChIP-R"`` so they drop
    into the same CSV alongside PY-IDR / Vanilla IDR / MaxRank / eCV.

    Like MaxRank and eCV, the ChIP-R statistic is deterministic given
    the data, so per-alpha rows differ only in the Sun-Cai cutoff.
    """
    import time as _time
    ...
    for i in range(num_replicates):
        ...
        sim = simulator(K=K, n=n, seed=seed, **extra)
        t0 = _time.perf_counter()
        idr = fit_chipr(np.asarray(sim.X), tie_method=tie_method)
        walltime = float(_time.perf_counter() - t0)
        idr_jax = _jnp.asarray(idr)
        for alpha in alphas:
            recovery = _evaluate_recovery(idr_jax, sim.true_Z, alpha=float(alpha))
